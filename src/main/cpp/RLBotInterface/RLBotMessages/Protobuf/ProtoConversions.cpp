...
namespace ProtoConversions {
	...
	IndexedPlayerInput* controllerStateToStruct(rlbot::api::ControllerState controllerState, int playerIndex)
	{
		IndexedPlayerInput* playerInput = new IndexedPlayerInput();
		playerInput->Index = playerIndex;

		playerInput->PlayerInput.Boost = controllerState.boost();
		playerInput->PlayerInput.Handbrake = controllerState.handbrake();
		playerInput->PlayerInput.Jump = controllerState.jump();
		playerInput->PlayerInput.Pitch = controllerState.pitch();
		playerInput->PlayerInput.Roll = controllerState.roll();
		playerInput->PlayerInput.Steer = controllerState.steer();
		playerInput->PlayerInput.Throttle = controllerState.throttle();
		playerInput->PlayerInput.Yaw = controllerState.yaw();
	...
	{
		return controllerStateToStruct(protoResult->controller_state(), protoResult->player_index());
	}
}
